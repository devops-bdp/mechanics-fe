...
import { showError, showSuccess, showConfirm, confirmDataChange } from '@/lib/swal';
...
export default function SuperAdminUsersPage() {
  ...
  const handleDownloadUsers = async () => {
    try {
      ...
      const blob = await apiClient.downloadUsersExcel(params);
      ...
      await showError(error.response?.data?.message || 'Failed to download users');
    } finally {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Confirm before saving changes (only for updates, not creates)
    if (editingUser) {
      const confirmResult = await confirmDataChange(
        `Are you sure you want to update user "${formData.firstName} ${formData.lastName}" (${formData.email})?`,
        'Confirm User Update'
      );
