...
export default function EditUnitModal({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        await showSuccess("Unit updated successfully!");
        onSuccess();
        onClose();
      } else {
        await showError(response.message || "Failed to update unit");
      }
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200 sticky top-0 bg-white">
          ...
          <p className="text-sm text-gray-500 mt-1">Unit ID: {unit.id}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          <div className="space-y-4">
            ...
            <div>
              ...
              {(formData.unitStatus === "BREAKDOWN" || formData.unitStatus === "INACTIVE") && (
                <p className="mt-2 text-xs text-blue-600 bg-blue-50 p-2 rounded">
                  ℹ️ Unit with {formData.unitStatus} status can have activities created.
                </p>
              )}
            </div>
          </div>
